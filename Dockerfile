...
VOLUME ["/opt/dxlmispservice-config"]
...
#       mapped to the path: /opt/dxlmispservice-config
#
# For example, specify a "-v" argument to the run command
# to mount a directory on the host as a data volume:
#
#   -v /host/dir/to/config:/opt/dxlmispservice-config
#
CMD ["python", "-m", "dxlmispservice", "/opt/dxlmispservice-config"]
